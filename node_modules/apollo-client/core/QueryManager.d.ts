...
export declare class QueryManager<TStore> {
    ...
    mutate<T>({mutation, variables, optimisticResponse, updateQueries: updateQueriesByName, refetchQueries, update: updateWithProxyFn, errorPolicy, fetchPolicy, context}: MutationOptions): Promise<FetchResult<T>>;
    ...
    private getObservableQueryPromises(includeStandby?);
    ...
    getCurrentQueryResult<T>(observableQuery: ObservableQuery<T>, optimistic?: boolean): any;
    ...
    private buildOperationForLink(document, variables, extraContext?);
}
